/**
 * This file was auto-generated by Fern from our API Definition.
 */
package com.basistheory.core;

public final class Environment {
    public static final Environment DEFAULT = new Environment("https://api.basistheory.com");

    public static final Environment US = new Environment("https://api.us.basistheory.com");

    public static final Environment EU = new Environment("https://api.eu.basistheory.com");

    public static final Environment TEST = new Environment("https://api.test.basistheory.com");

    private final String url;

    private Environment(String url) {
        this.url = url;
    }

    public String getUrl() {
        return this.url;
    }

    public static Environment custom(String url) {
        return new Environment(url);
    }
}
