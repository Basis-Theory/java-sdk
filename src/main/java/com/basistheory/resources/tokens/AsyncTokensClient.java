...
public class AsyncTokensClient {
    ...
    public CompletableFuture<SyncPagingIterable<Token>> list(TokensListRequest request, RequestOptions requestOptions) {
        ...
        CompletableFuture<SyncPagingIterable<Token>> future = new CompletableFuture<>();
        client.newCall(okhttpRequest).enqueue(new Callback() {
            @Override
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                try (ResponseBody responseBody = response.body()) {
                    if (response.isSuccessful()) {
                        ...
                        future.complete(new SyncPagingIterable<Token>(true, result, () -> {
                            try {
    ...
    public CompletableFuture<Token> create(CreateTokenRequest request, IdempotentRequestOptions requestOptions) {
        ...
        client.newCall(okhttpRequest).enqueue(new Callback() {
            ...
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                try (ResponseBody responseBody = response.body()) {
                    if (response.isSuccessful()) {
                        future.complete(ObjectMappers.JSON_MAPPER.readValue(responseBody.string(), Token.class));
                        return;
                    }
                    String responseBodyString = responseBody != null ? responseBody.string() : "{}";
                    try {
                ...
                try (ResponseBody responseBody = response.body()) {
                    if (response.isSuccessful()) {
                        ...
                        TokensListV2Request nextRequest = TokensListV2Request.builder()
                                ...
                                .build();
                        List<Token> result = parsedResponse.getData().orElse(Collections.emptyList());
                        future.complete(new SyncPagingIterable<Token>(startingAfter.isPresent(), result, () -> {
                            try {
                                return listV2(nextRequest, requestOptions).get();
                            } catch (InterruptedException | ExecutionException e) {
                                throw new RuntimeException(e);
                            }
                        }));
            ...
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                try (ResponseBody responseBody = response.body()) {
                    if (response.isSuccessful()) {
                        ...
                        future.complete(new SyncPagingIterable<Token>(startingAfter.isPresent(), result, () -> {
                            try {
                                return searchV2(nextRequest, requestOptions).get();
                            } catch (InterruptedException | ExecutionException e) {
                                throw new RuntimeException(e);
                            }
                        }));
