...
public class RawApplePayClient {
    ...
    public BasisTheoryApiHttpResponse<ApplePayCreateResponse> create(RequestOptions requestOptions) {
    ...
    public BasisTheoryApiHttpResponse<ApplePayToken> get(String id, RequestOptions requestOptions) {
        ...
        try (Response response = client.newCall(okhttpRequest).execute()) {
            ...
            if (response.isSuccessful()) {
                return new BasisTheoryApiHttpResponse<>(
                        ObjectMappers.JSON_MAPPER.readValue(responseBodyString, ApplePayToken.class), response);
            }
            try {
                switch (response.code()) {
                    case 401:
